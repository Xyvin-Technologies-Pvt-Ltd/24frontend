...
import type { UpdateSurveyData, SurveyQuestion, MultilingualField } from "@/types/survey"
...
const getLocalizedField = (text: any): MultilingualField => {
  if (typeof text === 'string') {
    return { en: text, ml: text }
  }
  if (typeof text === 'object' && text !== null) {
    return { en: text.en || '', ml: text.ml || '' }
  }
  return { en: '', ml: '' }
}

export function EditSurveyForm({ surveyId, onBack, onSave }: EditSurveyFormProps) {
  const { toasts, removeToast, success, error: showError } = useToast()
  const { data: surveyResponse, isLoading } = useSurvey(surveyId)
  ...
  useEffect(() => {
    if (surveyResponse?.data) {
      ...
      setFormData({
        survey_name: getLocalizedField(survey.survey_name),
        description: getLocalizedField(survey.description),
      ...
      if (survey.questions && survey.questions.length > 0) {
        setQuestions(survey.questions.map(q => ({
          _id: q._id,
          question_text: getLocalizedField(q.question_text),
          answer_type: q.answer_type,
          options: q.options?.map(opt => getLocalizedField(opt)) || [],
  ...
  const handleQuestionChange = (index: number, field: string, value: any) => {
    ...
    if (field === 'answer_type' && value === 'multiple_choice') {
      updatedQuestions[index].options = updatedQuestions[index].options?.length 
  ...
  const handleSubmit = async (status: "active" | "closed") => {
    try {
      ...
      showError(error.message || 'Failed to update survey')
    }
  }
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 pt-[100px] p-8 bg-gray-50 overflow-y-auto">
        ...
        <div className="flex items-center text-sm text-gray-600 mb-8">
          ...
          <span>Survey</span>
          <span className="mx-2">›</span>
          <span className="text-gray-900">Edit Survey</span>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-4 pt-4">
              ...
              <Button
                ...
                disabled={updateSurveyMutation.isPending}
                className="px-8 py-3 bg-gray-300 hover:bg-gray-400 text-gray-700 rounded-full"
              >
